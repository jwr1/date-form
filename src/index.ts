...
export type FormatTokens = Array<string | [string, number?, ...string[]]>;
...
export class DateForm {
  ...
  static codeMatch = /^%([^%a-z]*)([%a-z]+)/i;
  static flagMatch = /^([^%a-z])/i;
  static padMatch = /^([1-9]\d*)/;
  ...
  static parseFormat(input: string): FormatTokens {
    ...
    for (let i = 0; i < input.length; i++) {
      const str = input.slice(Math.max(0, i));
      const match = this.codeMatch.exec(str);
      if (match) {
        ...
        for (let j = 0; j < flagsMatch.length; j++) {
          const flagStr = flagsMatch.slice(Math.max(0, j));

          const padMatchA = this.padMatch.exec(flagStr);
          if (padMatchA) {
            const [wholePadMatch, padMatchG] = padMatchA;
            const padMatchGNum = Number(padMatchG);
            if (!Number.isNaN(padMatchGNum)) pad = padMatchGNum;
            j += wholePadMatch.length - 1;
          } else {
            const flagMatchA = this.flagMatch.exec(flagStr);
            if (flagMatchA) {
  ...
  static applyFormat(input: FormatTokens, date = new Date()): string {
    ...
    for (const token of input) {
      if (Array.isArray(token)) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.codes, codeMatch)) {
          const {
            gen: codeFunc,
            flag: defaultFlag,
            pad: defaultPad,
          } = this.codes[codeMatch];
          ...
          if (flagsMatch.length > 0) {
            ...
            if (typeof this.flags[defaultFlag] === 'function') {
              ...
              tokenOut = tokenOut.padStart(
                pad,
                this.flags[defaultFlag] as string,
              );
            }
          }
...
export const utils = {
  ...
  dayOfYear: ((d) =>
    ...
    86_400_000) as CodeGen, // 24 * 60 * 60 * 1000
  hour12: ((d) => ((d.getHours() + 11) % 12) + 1) as CodeGen,
  hour24: ((d) => d.getHours()) as CodeGen,
  meridiemLower: ((d) =>
    d.getHours() >= 12 ? names.meridiem.pm : names.meridiem.am) as CodeGen,
  meridiemUpper: ((d) =>
    d.getHours() >= 12 ? names.meridiem.PM : names.meridiem.AM) as CodeGen,
